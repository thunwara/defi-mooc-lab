...
describe("Liquidation 3", function () {
  it("test", async function () {
    ...
    const gasPrice = 0;
    // const debt_USDC = ethers.utils.parseUnits("8128956343", 0); // real world
    const debt_USDC = ethers.utils.parseUnits("8400", 6); // my profit
    ...
    const liquidationEvents = liquidationReceipt.logs.filter(
      (v) => v && v.topics &&
        v.address === "0x7d2768dE32b0b80b7a3454c06BdAc94A69DDc7A9" && Array.isArray(v.topics) &&
    ...
    console.log(
      `Profit from flash loan ${debt_USDC} USDC:`, utils.formatEther(profit), "ETH"
    );
